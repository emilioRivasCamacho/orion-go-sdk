...
func TestHealthchecks(t *testing.T) {
	...
	go svc.Listen(func() {
		...
		req := &Request{}
		req.SetPath("/"+id+"/status.am-i-up")

		genres := &Response{}
		svc.Call(req, genres)

		req = &Request{}
		req.SetPath("/"+id+"/status.good")
		...
		*req2.Params.Type = health.INTERNAL
		...
		req2.SetPath("/"+id+"/status.aggregate")
